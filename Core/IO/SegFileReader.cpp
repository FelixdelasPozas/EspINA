...
IOErrorHandler::STATUS SegFileReader::loadSegFile(QFileInfo file,
		IEspinaModel *model, IOErrorHandler *handler)
{
	...
	if (!espinaZip.open(QuaZip::mdUnzip))
	{
		if (handler)
			handler->error(
					"IOEspinaFile: Could not open file" + file.filePath());
	...
	while (hasFile)
	{
		...
		if (!espinaFile.open(QIODevice::ReadOnly))
		{
			if (handler)
				handler->error(
						"IOEspinaFile: Could not extract the file"
								+ file.filePath());
		...
		{
			IOErrorHandler::STATUS status = readSettings(espinaFile, model,
					handler);
		...
		if (file.fileName() == FILE_VERSION)
		{
			...
			if (versionNumber < SEG_FILE_COMPATIBLE_VERSION)
			{
				if (handler)
					handler->error(
							QObject::tr(
									"Invalid seg file version. File Version=%1, current Version %2").arg(
									versionNumber).arg(SEG_FILE_VERSION));
				removeTemporalDir();
				espinaFile.close();
		...
		else
		{
			...
			{
				int i = 0;
				Channel::ExtensionList extensions =
						model->factory()->channelExtensions();
			...
			{
				int i = 0;
				Segmentation::InformationExtensionList extensions =
						model->factory()->segmentationExtensions();
				while (!extensionFile && i < extensions.size())
				{
			...
			if (!extensionFile)
			{
				QFileInfo destination = temporalDir.absoluteFilePath(
						file.filePath());
				...
				if (!destinationFile.open(
						QIODevice::WriteOnly | QIODevice::Truncate))
				{
					if (handler)
						handler->warning(
								"IOEspinaFile::loadFile: could not create file "
										+ destinationFile.fileName() + " in "
										+ temporalDir.path());
				}
	...
	if (taxonomy.get() == NULL || traceContent.isEmpty())
	{
		if (handler)
			handler->error(
					"IOEspinaFile::loadFile: could not load taxonomy and/or trace files");
		removeTemporalDir();
		return IOErrorHandler::IO_ERROR;
	}
	...
	status =
			loadSerialization(model, trace, temporalDir, handler) ?
					IOErrorHandler::SUCCESS : IOErrorHandler::IO_ERROR;
...
//-----------------------------------------------------------------------------
IOErrorHandler::STATUS SegFileReader::saveSegFile(QFileInfo file,
		IEspinaModel *model, IOErrorHandler *handler)
{
...
QDir temporalDir = QDir::tempPath();
QString path = QString("espina%1io%1%2").arg(QDir::separator()).arg(
		file.baseName());
temporalDir.mkpath(path);
temporalDir.cd(path);

QString temporalSegFileName = temporalDir.path() + QDir::separator()
		+ file.fileName();
...
if (!zip.open(QuaZip::mdCreate))
{
	if (handler)
		handler->error(
				"IOEspinaFile::saveFile" + zFile.fileName()
...
if (zip.getZipError() != UNZ_OK)
{
	if (handler)
		handler->error(
				"IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: Failed to close"
						+ zFile.fileName() + "zip file");
...
{
	qDebug() << "IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: remove"
			<< temporalSegFileName << "file";
	removeTemporalDir(temporalDir);
return IOErrorHandler::IO_ERROR;
}
}
...
{
qDebug() << "IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: Failed to rename"
	<< temporalSegFileName << "to" << file.absoluteFilePath();
removeTemporalDir(temporalDir);
return IOErrorHandler::IO_ERROR;
}
...
void SegFileReader::serializeRelations(IEspinaModel *model, ostream &stream,
...
//-----------------------------------------------------------------------------
bool SegFileReader::zipFile(QString fileName, const QByteArray &content,
QuaZipFile& zFile)
{
...
{
qWarning() << "IOEspinaFile::zipFile(): Could not open " << fileName << "inside"
...
{
qWarning() << "IOEspinaFile::zipFile(): Could not store the content in"
<< fileName << "inside" << zFile.getFileName() << ". Code error:"
<< zFile.getZipError();
return false;
}
zFile.close();
...
<< "inside" << zFile.getFileName() << ". Code error:" << zFile.getZipError();
return false;
}
return true;
}
...
if (!node->properties().contains("Dim_X")
		|| !node->properties().contains("Dim_Y")
		|| !node->properties().contains("Dim_Z"))
{
	qWarning() << "Taxonomy" << node->name() << "is missing some properties.";
	TaxonomySPtr defaultTaxonomy = IOTaxonomy::openXMLTaxonomy(
			":/espina/defaultTaxonomy.xml"); // TODO: Remove dependency to be used as independent lib
	TaxonomyElementSPtr defaultNode = defaultTaxonomy->element(
			node->qualifiedName());
...
void IOTaxonomy::writeTaxonomyElement(TaxonomyElementSPtr node,
...
IOErrorHandler::STATUS SegFileReader::removeTemporalDir(QDir temporalDir)
{
	...
	if (!result)
	{
		qWarning() << "SegFileReader: Failed to remove temporal directory"
				<< dirName;
	return IOErrorHandler::IO_ERROR;
}
...
bool parseVertex(RelationshipGraphPtr input, RelationshipGraph::Vertex &v,
	IEspinaModel *model, QDir tmpDir,
...
if (!v.item)
{
	...
	switch (RelationshipGraph::type(v))
	{
		case SAMPLE:
		{
			...
			SampleSPtr sample = factory->createSample(v.name.c_str(),
					v.args.c_str());
		...
		case CHANNEL:
		{
			...
			ModelItem::Arguments extArgs(
					args.value(ModelItem::EXTENSIONS, QString()));
			...
			RelationshipGraph::Vertices ancestors = input->ancestors(v,
					link[0]);
			...
			{
				RelationshipGraph::Vertices successors = input->succesors(v,
						QString());
				if (!successors.empty())
					return false;
		...
		case FILTER:
		{
			...
			QStringList inputLinks = args[Filter::INPUTS].split(",",
			...
			FilterSPtr filter = factory->createFilter(v.name.c_str(), inputs,
					args);
...
//-----------------------------------------------------------------------------
bool SegFileReader::loadSerialization(IEspinaModel *model, istream &stream,
	QDir tmpDir, IOErrorHandler *handler, RelationshipGraph::PrintFormat format)
{
...
foreach(RelationshipGraph::Edge e, input->edges()){ //Should store just the modelitem?
...
foreach(FilterSPtr filter, model->filters())filter->upkeeping();
