                                                  ...
                                                  IOErrorHandler *handler)
{
  ...
  while (hasFile)
  {
    ...
    if (file.fileName() == FILE_VERSION)
    {
      ...
      if (versionNumber < SEG_FILE_COMPATIBLE_VERSION)
      {
        if (handler)
          handler->error(
              QObject::tr("Invalid seg file version. File Version=%1, current Version %2").arg(versionNumber).arg(
                  SEG_FILE_VERSION));
        removeTemporalDir();
        espinaFile.close();
        ...
        else
        {
          ...
          if (!extensionFile)
          {
            ...
            if (!destinationFile.open(QIODevice::WriteOnly | QIODevice::Truncate))
            {
              if (handler)
                handler->warning(
                    "IOEspinaFile::loadFile: could not create file " + destinationFile.fileName() + " in "
                        + temporalDir.path());
            }
  ...
  status = loadSerialization(model, trace, temporalDir, handler) ? IOErrorHandler::SUCCESS : IOErrorHandler::IO_ERROR;
                                                  ...
                                                  IOErrorHandler *handler)
{
  ...
  {
    if (handler)
      handler->error("IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: Failed to close" + zFile.fileName() + "zip file");
  ...
  if (temporalDir.exists(file.absoluteFilePath()))
  {
    ...
    {
      qDebug() << "IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: remove" << temporalSegFileName << "file";
      removeTemporalDir(temporalDir);
      return IOErrorHandler::IO_ERROR;
    }
  }
  ...
  {
    qDebug() << "IOEspinaFile::saveFile IOErrorHandler::IO_ERROR: Failed to rename" << temporalSegFileName << "to"
...
//-----------------------------------------------------------------------------
bool SegFileReader::zipFile(QString fileName,
