...
//-----------------------------------------------------------------------
SegmentationExtensionSPtr SegmentationNotesFactory::createExtension(const SegmentationExtension::Type &type,
                                                                    const SegmentationExtension::InfoCache &cache,
                                                                    const State& state) const
{
  SegmentationExtensionSPtr extension = nullptr;

  if(type == SegmentationNotes::TYPE)
  {
  ...
  if(!extension || !extension.get())
  {
    ...
    auto details = QObject::tr("SegmentationNotesFactory::createExtension() -> ") + message;
