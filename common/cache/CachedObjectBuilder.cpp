...
//------------------------------------------------------------------------
pqFilter* CachedObjectBuilder::loadFile(const QFileInfo file)
{
  Cache::Index id = file.fileName();
  pqFilter *reader = getFilter(id);
  if (NULL == reader)
  {
    vtkImageAlgorithm *source = NULL;
    QString ext = file.suffix();
    if ("mhd" == ext || "mha" == ext)
    {
	vtkMetaImageReader *miReader = vtkMetaImageReader::New();
	miReader->SetFileName(file.absoluteFilePath().toAscii());
	source = miReader;
    }
    else if ("tif" == ext || "tiff" == ext)
    {
      vtkTIFFReader *tiffReader = vtkTIFFReader::New();
      tiffReader->SetFileName(file.absolutePath().toAscii());
      source = tiffReader;
    }
    else
    {
...
//   }
// 
//   vtkImageAlgorithm *proxy = createSMFilter(group, name, args);
...
vtkImageAlgorithm *CachedObjectBuilder::createSMFilter(const QString group, const QString name, const pqFilter::Arguments args)
{
  Q_ASSERT(false);
...
pqFilter* CachedObjectBuilder::registerFilter(const QString id, vtkImageAlgorithm* source)
{
