...
vtkStandardNewMacro(vtkErodeImageFilter);

const unsigned int LABEL_VALUE = 255;

vtkErodeImageFilter::vtkErodeImageFilter()
: Radius(3)
{
...
int vtkErodeImageFilter::RequestData(vtkInformation* request,
...
/*
  ...
  vtkDebugMacro(<< "Compute Image Erode");
  typedef itk::BinaryBallStructuringElement<ImageType::PixelType, 3> StructuringElementType;
  typedef itk::ErodeObjectMorphologyImageFilter<ImageType, ImageType, StructuringElementType> bmcifType;
  ...
  bmcif->SetObjectValue(LABEL_VALUE);
