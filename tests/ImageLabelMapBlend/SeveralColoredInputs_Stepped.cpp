...
int SeveralColoredInputs_Stepped(int argc, char **argv)
{
...
//   blender->SetNumberOfThreads(3);
  ...
  std::string outFileName("SeveralInputs.mhd");
  writer->SetFileName(outFileName.c_str());
  writer->SetInputConnection(blender->GetOutputPort());
  writer->Write();
    
  bool failed = fileDiff(stackPath.filePath("tests/ImageLabelMapBlend/SeveralInputs.mhd"),
			 "SeveralInputs.mhd") || 
		fileDiff(stackPath.filePath("tests/ImageLabelMapBlend/SeveralInputs.zraw"),
			 "SeveralInputs.zraw");
  
  // In case the test fails, we may want to analyze the output
  if (!failed)
  {
    remove("SeveralInputs.mhd");
    remove("SeveralInputs.zraw");
  }
