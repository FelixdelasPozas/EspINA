...
vtkStandardNewMacro(vtkTubeSource);

vtkTubeSource::vtkTubeSource(int res)
{
  this->Resolution = res;
  this->BottomRadius = 0.5;
  this->TopRadius = 0.5;
  this->Capping = 0;
  this->BottomCenter[0] = this->BottomCenter[1] = this->BottomCenter[2] = 0.0;
  this->TopCenter[0] = 0.0;
  this->TopCenter[1] = 1.0;
  this->TopCenter[2] = 0.0;

  this->transform = vtkSmartPointer<vtkTransform>::New();

  this->pdTransform = vtkSmartPointer<vtkTransformPolyDataFilter>::New();
  this->pdTransform->SetTransform(this->transform);

  this->SetNumberOfInputPorts(0);
}

int vtkTubeSource::RequestData(vtkInformation *vtkNotUsed(request),
    ...
    vtkInformationVector *outputVector)
{
  ...
  double bottomcenter[3] = { 0,0,0 };
  double topcenter[3] = { 0,1,0 }; // tube build in y axis direction
  double height = sqrt(vtkMath::Distance2BetweenPoints(this->TopCenter, this->BottomCenter));
  ...
  for (i = 0; i < this->Resolution; i++)
  {
    ...
    xbot[0] = (nbot[0] * this->TopRadius) + bottomcenter[0];
    xtop[0] = (ntop[0] * this->BottomRadius) + topcenter[0];
    tcbot[0] = tctop[0] = fabs(2.0 * i / this->Resolution - 1.0);

    // y coordinate
    xbot[1] = 0.5 * height + bottomcenter[1];
    xtop[1] = -0.5 * height + topcenter[1];
    nbot[1] = ntop[1] = 0.0;
    tcbot[1] = 0.0;
    tctop[1] = 1.0;
    ...
    xbot[2] = (nbot[2] * this->TopRadius) + bottomcenter[2];
    xtop[2] = (ntop[2] * this->BottomRadius) + topcenter[2];
  ...
  if (this->Capping)
  {
    ...
    {
      // x coordinate
      xbot[0] = this->TopRadius * cos(i * angle);
      xtop[0] = this->BottomRadius * cos(i * angle);
      nbot[0] = ntop[0] = 0.0;
      tcbot[0] = tctop[0] = xbot[0];
      xbot[0] += bottomcenter[0];
      xtop[0] += topcenter[0];
      ...
      nbot[1] = 1.0;
      ntop[1] = -1.0;
      xbot[1] += bottomcenter[1];
      xtop[1] += topcenter[1];

      // z coordinate
      xbot[2] = -this->TopRadius * sin(i * angle);
      xtop[2] = -this->BottomRadius * sin(i * angle);
      tcbot[1] = tctop[1] = xbot[2];
      xbot[2] += bottomcenter[2];
      xtop[2] += topcenter[2];
      nbot[2] = 0.0;
      ntop[2] = 0.0;
...
void vtkTubeSource::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "Resolution: " << this->Resolution << "\n";
  os << indent << "Bottom Radius: " << this->BottomRadius << "\n";
  os << indent << "Top Radius: " << this->TopRadius << "\n";
  os << indent << "Bottom Center: (" << this->BottomCenter[0] << ", " << this->BottomCenter[1]  << ", " << this->BottomCenter[2] << " )\n";
  os << indent << "Top Center: (" << this->TopCenter[0] << ", " << this->TopCenter[1]  << ", " << this->TopCenter[2] << " )\n";
