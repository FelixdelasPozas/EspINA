...
void SliceBlender::setBackground ( IRenderable* background )
{
  ...
  vtkSMIntVectorProperty *sliceMode =
    vtkSMIntVectorProperty::SafeDownCast(m_bgSlicer->getProxy()->GetProperty("SliceMode"));
  ...
  vtkSMProxyProperty *lut = vtkSMProxyProperty::SafeDownCast(m_bgMapper->getProxy()->GetProperty("LookupTable"));
  if (lut)
  {
...
void SliceBlender::addSegmentation ( IRenderable* seg )
{
  ...
  vtkSMIntVectorProperty *sliceMode = vtkSMIntVectorProperty::SafeDownCast( slicer->getProxy()->GetProperty("SliceMode"));
  if (sliceMode)
  {
    sliceMode->SetElements1(5+m_plane);
  }
  ...
  */
  ...
  if (segLUT)
  {
	...
	colorSpace->SetElements1(0);
      ...
      vtkSMDoubleVectorProperty *rgbs = vtkSMDoubleVectorProperty::SafeDownCast(segLUT->getProxy()->GetProperty("RGBPoints"));
      if (rgbs)
      {
	// TODO: Use segmentation's information
