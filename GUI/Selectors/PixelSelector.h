...
{
  /** \class PixelSelector
   * \brief Returns the pixel coordinate clicked on the selection items if one is clicked of nothing if nothing is selected.
   *
   */
   ...
   * \brief Returns the pixel coordinates of the value closest to the specified one in the window region.
   *
   */
  ...
  : public PixelSelector
  {
    ...
    private:
      QSize *m_window;    /** window size.      */
      int    m_bestPixel; /** best pixel value. */
  };
